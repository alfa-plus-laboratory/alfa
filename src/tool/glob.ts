/**
 * glob tool: find paths by file-name pattern, newest modification first (recently changed
 * files tend to be more relevant).
 *
 * Dedicated path search keeps the model from falling back to find/ls in bash,
 * where filesystem access is harder to govern.
 */
import { z } from "zod"
...
export const GlobTool: ToolDef<Args> = {
  ...
  async execute(args, ctx) {
    ...
    // rawPattern: the glob tool's pattern is an explicit glob, so keep its literal semantics
    // (unlike grep's include — that one has to line up with ripgrep's basename semantics)
