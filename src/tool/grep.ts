...
import { authorizePath, canonicalPath, within } from "../fs/guard.ts"
...
export const GrepTool: ToolDef<Args> = {
  ...
  async execute(args, ctx) {
    ...
    const requested = args.path
      ? await authorizePath(args.path, ctx, "read")
      : ctx.cwd

    let singleFile = false
    let searchDir = requested
    try {
      if (!statSync(requested).isDirectory()) { searchDir = dirname(requested); singleFile = true }
    } catch {
    ...
    const result = await grep({
      ...
      authorizeFile: async path => {
        ...
        if (singleFile && real !== canonicalPath(requested)) return false
        if (!within(real, canonicalPath(searchDir))) await authorizePath(real, ctx, "read")
        await ctx.ask({ permission: "read", patterns: [workspacePattern(real, ctx.root)] })
        return true
      },
      limit: RESULT_LIMIT,
    ...
    // Matches in the same file are printed back to back; on a new file, a blank line and
    // then the path
    const out: string[] = [
