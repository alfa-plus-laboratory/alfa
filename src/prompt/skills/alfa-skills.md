...
| `.claude/skills/` in the repository | **also loaded**, like `.alfa/skills/`. Skills written for other agents work here unchanged. |
| `~/.claude/skills/` | **on the shelf**, next to `{{libraryDir}}`. What the user installed for another agent is not switched on everywhere here. |

Same name in more than one place, most specific first: project `.alfa` → project `.claude` → user `.alfa` → built in. Write into `.alfa/` when you want to override something that arrived from elsewhere. On the shelf, `{{libraryDir}}` wins over `~/.claude/skills/`.

When the user asks why a skill they have for another agent is missing here, it is most likely in `~/.claude/skills/`: it is on the shelf (`skill` with `action: "library"`). If they want it everywhere, it goes in `{{skillsDir}}`; if only in this project, in `.alfa/skills/`.
