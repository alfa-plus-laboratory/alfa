...
**每进一个新文件夹**还有一张全屏的卡片,问两句:屏幕怎么排(每一条旁边就画着它长
什么样),以及这个文件夹信不信任。两个答案都存进**你的 config**,仓库里一个字都不写。
想用默认值就连按两下回车,之后随时 `/setting` 改。
...
- **派子 agent 出去** —— `task` 派一个带独立上下文窗口的子 agent。适合那种"要读遍
  半个仓库、但答案只有一段话"的问题。`/agentflow` 把上限抬到一百个,并让它默认摊开来
  并行干;它照旧自己改文件、自己跑命令,变的是同时能有多少事在动。
...
授权框上 `⏎` 放行、`esc` 拒绝,而且这两个**排在字母 `y` / `n` 前面**。顺序是有讲究
的:中日韩输入法坐在键盘和终端中间,字母键会被它吃掉 —— 按下 `y` 弹出来的是候选词
窗口,这个程序一个字节都收不到。`⏎` 和 `esc` 是它碰不到的两个。万一候选词上屏落到
了框上,它会说一句怎么按,而不是静悄悄地不动。
...
`ctrl-y` —— 或者点小机器人那条横线右端的 `[⧉ copy]` —— 列出可以复制的东西,内容
**从会话库里取原文**:最后那段回答里的每个代码块、整段回答、你刚才那句话、或者整场
对话。那条线就在它刚说的那段话正上方,而那正是你想把它拿走时眼睛落着的地方。走
...
- **`/setting`** 把上面这些摆在一屏上:视图、侧栏、信任、权限模式、扩展思考、
  agentflow、自动压缩、语言、模型,以及你的 API 密钥。左右键就地改,单条命令照旧能用。
- **`/model`** 不带参数直接打开模型清单,光标已经停在当前用的那个上。换模型不清历史。
